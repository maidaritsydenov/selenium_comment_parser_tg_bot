...
class _VkChromeClientPipeline(BrowserClientBasePipeline):
    ...
    def _goto_live_broadcasts(self):
        ...
        '''
        live_broadcast_selector = self._find_live_broadcast_selector(
            'yt-formatted-string:contains("")'
        )
        wait_broadcast_selector = ''
        self._click_and_wait(live_broadcast_selector, wait_broadcast_selector)

    def _find_live_broadcast_selector(self):
        '''
        Найти на странице кликабельную ссылку на `live`-трансляции.
        '''
        pass
