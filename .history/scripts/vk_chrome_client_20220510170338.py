...
class _VkChromeClientPipeline(BrowserClientBasePipeline):
    ...
    def _save_new_chat_messages(self):
        if self._chat_messages_container is None:
            self._chat_messages_container = ChatMessagesContainer()
        existing_messages = 
        self._chat_messages_container.add_messages(existing_messages)
