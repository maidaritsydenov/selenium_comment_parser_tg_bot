'''
Бот telegram.

1)
'''
from collections import defaultdict
from queue import Queue
from time import sleep
from telegram.ext import Updater, CommandHandler
from settings import (
    TELEGRAM_TOKEN, PHRASES_FILE_PATH, SCREEN_FILE_PATH, ALLOWED_USERS
)


class BotApp:
    _JOB_INTERVAL = 5.0

    def __init__(self):
        self._filters_by_users = defaultdict(lambda: [])
        self._phrases_by_users = defaultdict(lambda: Queue())

    def _global_timer_add_users_phrases(self, job_context):
        # Прочитать файл
        # Заполнить очередь каждого пользователя - новыми строками из файла
        line = None
        with open(PHRASES_FILE_PATH, 'r', encoding='UTF-8') as f:
            while True:
                if line:
                    for chat_id in self._phrases_by_users.keys():
                        self._phrases_by_users[chat_id].put(line)
                    line = f.readline()
                else:
                    while not line:
                        sleep(self._JOB_INTERVAL)
                        line = f.readline()

    def _pop_user_phrases(self, chat_id):
        # Обработать очередь пользователя
        # Выдать только те items, которые удовлетворяет
        # self._filters_by_users
        # А всю очередь - очистить
        output_phrases = []
        user_queue = self._phrases_by_users[chat_id]
        user_filters = self._filters_by_users[chat_id]
        while not user_queue.empty():
            phrase = user_queue.get()
            # # Если фильтры не установлены - то просто выдаём все сообщений,
            # # если установлены - то только те,
            # которые удовлетворяют фильтрам.
            # if len(user_filters) > 0:
            for user_filter in user_filters:
                if user_filter in phrase:
                    output_phrases.append(phrase)
            # else:
            #     output_phrases.append(phrase)
        return output_phrases

    def _timer_check_filters(self, context):
        update = context.job.context
        chat_id = update.message.chat_id
        found_phrases = self._pop_user_phrases(chat_id)
        for found_phrase in found_phrases:
            update.message.reply_text(found_phrase)
        if len(found_phrases) > 0:
            self._send_screen(update)

    def _check_allow_user(f):
        def inner(self, update, context):
            if update.effective_user.username in ALLOWED_USERS:
                return f(self, update, context)
            else:
                update.message.reply_text('You are not allowed yet.')
        return inner

    @_check_allow_user
    def _callback_start(self, update, context):
        update.message.reply_text('Welcome!')
        context.job_queue.run_repeating(
            self._timer_check_filters,
            self._JOB_INTERVAL,
            context=update,
        )

    @_check_allow_user
    def _callback_stop(self, update, context):
        context.job_queue.stop()
        update.message.reply_text('Bye!')

    @_check_allow_user
    def _callback_add_filter(self, update, context):
        chat_id = update.message.chat_id
        filter_word = self._get_command_arg(update.message.text)
        if len(filter_word) > 0:
            self._filters_by_users[chat_id].append(filter_word)
            self._reply_current_filters(update)
        else:
            update.message.reply_text('`Filters` must not be empty.')

    @_check_allow_user
    def _callback_remove_filter(self, update, context):
        chat_id = update.message.chat_id
        command_arg = self._get_command_arg(update.message.text)
        user_filter_list = self._filters_by_users[chat_id]
        if command_arg.isnumeric():
            filter_num = int(command_arg) - 1
        else:
            filter_num = user_filter_list.index(command_arg)
        del user_filter_list[filter_num]
        self._reply_current_filters(update)

    @_check_allow_user
    def _callback_show_filters(self, update, context):
        self._reply_current_filters(update)

    def _reply_current_filters(self, update):
        chat_id = update.message.chat_id
        user_filters = self._filters_by_users[chat_id]
        user_filters = ['%d. "%s"' %
                        (i + 1, user_filter) \
                        for i, user_filter in enumerate(user_filters)]
        output_user_filters = '\n'.join(user_filters)
        if len(user_filters) > 0:
            update.message.reply_text(output_user_filters)
        else:
            update.message.reply_text('No filters.')

    def _send_screen(self, update):
        update.message.reply_photo(photo=open(SCREEN_FILE_PATH, 'rb'))

    @staticmethod
    def _get_command_arg(text):
        return ''.join(text.split()[1:])

    def init(self):
        '''
        Регистрация команд бота и добавление их обработчика.
        '''
        updater = Updater(TELEGRAM_TOKEN)
        dispatcher = updater.dispatcher
        dispatcher.add_handler(
            CommandHandler("start", self._callback_start, pass_job_queue=True))
        dispatcher.add_handler(
            CommandHandler("stop", self._callback_stop, pass_job_queue=True))
        dispatcher.add_handler(
            CommandHandler("add", self._callback_add_filter))
        dispatcher.add_handler(
            CommandHandler("show", self._callback_show_filters))
        dispatcher.add_handler(
            CommandHandler("remove", self._callback_remove_filter))

        updater.job_queue.run_once(
            self._global_timer_add_users_phrases, when=0)

        updater.start_polling()
        updater.idle()


if __name__ == '__main__':
    BotApp().init()
